...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className={cn(
        ...
        <div className="flex items-center gap-4 group">
          <div className="relative">
            <div
              ...
              style={{ backgroundColor: `${distributorColor}20` }}
            >
              <img
                src={belemLogo}
                alt="Logo Belém TI"
            ...
            <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-zinc-950 rounded-lg flex items-center justify-center border-2 border-zinc-900 group-hover:bg-blue-500 transition-colors">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[8px] font-bold uppercase tracking-widest text-zinc-500">Sistema PDV</span>
              <div className="w-1 h-1 bg-zinc-700 rounded-full" />
              <span className="text-[9px] font-bold text-zinc-600">v2.4.0</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Navigation */}
      <div className="px-5 py-8 space-y-1.5 overflow-y-auto max-h-[calc(100vh-210px)] custom-scrollbar">
        {menuItems.map((item) => {
            ...
            <button
              ...
              className={cn(
                ...
                isActive
                  ...
                  : (isDarkMode ? "text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900/50" : "text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50")
              )}
            >
              <div className={cn(
              ...
              {!isActive && (
                <ChevronRight className="ml-auto opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 text-zinc-600" size={14} />
              )}
            </button>
          );
        })}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-zinc-500/10">
        <div className="flex items-center gap-3 px-2 mb-4">
          <div className="w-8 h-8 rounded-lg bg-zinc-800 flex items-center justify-center text-xs font-bold text-white uppercase">
            {currentUser?.name.charAt(0)}
          </div>
          <div className="flex flex-col min-w-0">
            <span className="text-xs font-bold text-zinc-400 truncate uppercase">{currentUser?.name}</span>
