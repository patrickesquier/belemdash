    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm font-medium mt-1 uppercase tracking-tight">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat, i) => (
          ...
          >
            ...
            <h3 className="text-[11px] font-black uppercase tracking-widest text-zinc-500 mb-1">{stat.label}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className={cn(
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-xs text-zinc-500 font-medium">Desempenho dos últimos 7 dias</p>
            </div>
          </div>
        ...
        <div className={cn(
          ...
          <h3 className="text-sm font-bold uppercase tracking-tight mb-8 text-center text-zinc-500">Formas de Pagamento</h3>
                      ...
                      {payload?.map((entry: any, index: number) => (
                        <div key={index} className="flex items-center justify-between text-[11px] font-bold">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-zinc-400 uppercase tracking-widest">{entry.value}</span>
                          </div>
                          <span className={isDarkMode ? "text-white" : "text-zinc-900"}>
                            {stats.paymentData[index].value}
                          </span>
                        </div>
                      ))}
                    </div>
        ...
        <div className={cn(
          ...
          <div className="space-y-4">
            {stats.topProducts?.map((product: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-zinc-500/5 group hover:bg-zinc-500/10 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-zinc-500/10 flex items-center justify-center text-xs font-black text-zinc-500">
                    {i + 1}
                  </div>
                ...
                <div className="text-right">
                  ...
                  <p className="text-[9px] font-bold text-zinc-500 uppercase">Unidades</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {stats.lowStockProducts?.map((product: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-red-500/[0.03] border border-red-500/10 group hover:bg-red-500/[0.06] transition-colors">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[9px] font-bold text-zinc-500 uppercase">Restantes</p>
                </div>
              </div>
            ))}
            {stats.lowStock === 0 && (
